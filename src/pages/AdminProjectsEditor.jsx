...
export default function AdminProjectsEditor() {
  ...
  const handleSubmit = async (e) => {
    ...
    for (const img of imagens) {
      if (img.file) {
        ...
        const fileName = `project-${Date.now()}-${img.file.name}`;
    ...
    const { error } = await supabase.from("projects_content").upsert(dataToSend);
    setLoading(false);
  ...
  const handleEdit = (proj) => {
    ...
    const imgs = Array.from({ length: 15 }, (_, i) => {
      const url = proj[`image${i + 1}_url`];
      if (!url) return null;
      return {
        file: null,
        url,
        caption_pt: proj[`image${i + 1}_caption_pt`] || "",
        caption_en: proj[`image${i + 1}_caption_en`] || ""
      };
    }).filter(Boolean);
  ...
  const handleDelete = async (id) => {
    ...
    const { error } = await supabase.from("projects_content").delete().eq("id", id);
    if (!error) fetchProjetos();
  };

  return (
    <div className="max-w-3xl mx-auto text-texto font-sans bg-admin p-6 rounded shadow">
      <h2 className="text-xl font-bold mb-4">{form.id ? "Editar Projeto" : "Adicionar Projeto"}</h2>
        ...
        <div>
          ...
          {imagens.map((img, i) => (
            <div key={i} className="mt-4 border p-2 rounded bg-white relative">
              {img.file ? (
                <img src={URL.createObjectURL(img.file)} alt={`preview-${i}`} className="h-32 object-cover mb-2 rounded" />
              ) : (
                <img src={img.url} alt={`preview-${i}`} className="h-32 object-cover mb-2 rounded" />
              )}
      ...
      <hr className="my-8" />
      <h2 className="text-lg font-bold mb-4">Projetos Existentes</h2>
      <ul className="space-y-6">
        {projetos.map((proj) => (
          <li key={proj.id} className="bg-white text-black rounded p-4 shadow">
            ...
            <div className="flex gap-4">
              ...
              <button onClick={() => handleDelete(proj.id)} className="bg-texto text-admin px-4 py-2 rounded hover:bg-gray-800">Apagar</button>
            </div>
          </li>
        ))}
      </ul>
